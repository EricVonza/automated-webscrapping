...
def format_match_vs(raw_text: str) -> str:
    ...
    if len(words) > 2:
        mid = len(words) // 2
        return f"{' '.join(words[:mid])} vs {' '.join(words[mid:])}"
...
def extract_timer(html):
    ...
    for t in soup.find_all(class_='c-events-scoreboard__subitem'):
        ...
        timers.append(
            f"{tm.text.strip() if tm else '00:00'} | {q.text.strip() if q else 'No Quarter'}"
        )
...
def send_telegram_message(msg):
    try:
        r = requests.post(T_URL, data={'chat_id': ROOM_ID, 'text': msg}, timeout=10)
...
def main():
    ...
    while True:
        ...
        for match, (t1, t2), timer in zip(matches, games, timers):
            ...
            game_alerts_sent.setdefault(match, 0)
            if game_alerts_sent[match] >= MAX_ALERTS_PER_GAME:
                continue
            ...
            if prev_q is not None and get_game_minute(timer) < cutoff:
                try:
                ...
                if (q1 < 12 or q2 < 12) and game_alerts_sent[match] < MAX_ALERTS_PER_GAME:
                    send_telegram_message(
                        f"⚠️ Low Quarter Alert\n{match}\n{timer}\nQ{prev_q + 1}: T1={q1} vs T2={q2}"
                    )
                    game_alerts_sent[match] += 1
                    continue
            ...
            if "2nd quarter" in timer_lower:
                ...
                if first_q_sum < 50 and any(x in timer for x in ["12:5", "13:0", "13:1", "16:5", "17:", "07:", "06:"]):
                    ...
                    if est_2q <= 45 and game_alerts_sent[match] < MAX_ALERTS_PER_GAME:
                        send_telegram_message(
                            f"📉 2Q UNDER ALERT\n{match}\n{timer}\nEstimated 2Q pts: OV{est_2q}"
                        )
                        game_alerts_sent[match] += 1

        # -------- HEARTBEAT --------
        if time.time() - last_heartbeat >= 15:
            logger.info(f"Heartbeat ✅ | Scanned {scanned_games} games")
            last_heartbeat = time.time()
